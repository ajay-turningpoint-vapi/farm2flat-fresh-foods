...
const AddOnsModal = ({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[95vw] max-w-[95vw] sm:max-w-2xl max-h-[90vh] overflow-y-auto p-3 sm:p-6">
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 sm:gap-3 md:gap-4 py-2 sm:py-4">
          {addOns.map((addOn) => {
            const isAdded = addedItems.includes(addOn.id);
            return (
              <div
                key={addOn.id}
                className={`bg-card rounded-lg sm:rounded-xl border p-2 sm:p-3 text-center transition-all ${
                  isAdded
                    ? "border-primary bg-primary/5"
                    : "border-border/50 hover:shadow-md"
                }`}
              >
                <div className="w-12 h-12 sm:w-16 sm:h-16 mx-auto mb-1.5 sm:mb-2 rounded-lg overflow-hidden bg-secondary/30">
                  <img
                    src={addOn.image}
                    alt={addOn.name}
                ...
                <p className="text-primary font-bold text-xs sm:text-sm mb-2 sm:mb-3">
                  ₹{addOn.price}/{addOn.unit}
                </p>
                <Button
                  size="sm"
                  variant={isAdded ? "default" : "outline"}
                  className="w-full text-[10px] sm:text-xs h-9 sm:h-8"
